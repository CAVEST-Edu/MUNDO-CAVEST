...
// 2. Sua configuração (Mantenha sempre esta constante no topo)
...
// 3. Função principal atualizada para buscar na Nuvem
async function carregarQuestoes(materia, assunto) {
    ...
    try {
        // Busca no Firebase filtrando por matéria e assunto
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            const idDoc = docSnap.id; // ID único do Firebase para não dar conflito

            const div = document.createElement("div");
            div.className = "card-questao"; 
            div.style.cssText = "background: #1e293b; padding: 25px; border-radius: 12px; margin-bottom: 30px; text-align: left; border: 1px solid #334155;";
            ...
            let resolucaoTag = q.resolucao ? `
                <div id="res-${idDoc}" style="display:none; margin-top:15px; padding:15px; background:#0f172a; border-radius:8px; border-left: 4px solid #facc15;">
            ...
            div.innerHTML = `
                ...
                <button onclick="verificarResposta('${idDoc}', '${q.correta}')" style="margin-top:25px; width: 100%; padding: 15px; background:#22c55e; color:white; border:none; border-radius:8px; cursor:pointer; font-weight:bold; font-size: 16px; transition: 0.3s;">Confirmar Resposta</button>
...
// 4. Torna a função global para o HTML conseguir chamar
window.verificarResposta = (id, correta) => {
    ...
    if (!selecionada) {
        alert("Comandante, selecione uma alternativa!");
        return;
    }
    ...
    if (selecionada.value === correta) {
        ...
        msg.innerText = `❌ Não foi dessa vez. A correta era (${correta.toUpperCase()})`;
